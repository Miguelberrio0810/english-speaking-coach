...
const SKILL_META: Record<Skill, { icon: string; color: string; bg: string; border: string }> = {
  speaking:  { icon: '🎙️', color: 'text-violet-400', bg: 'bg-violet-500/15',  border: 'border-violet-500/30' },
  listening: { icon: '🎧', color: 'text-amber-400',  bg: 'bg-amber-500/15',   border: 'border-amber-500/30'  },
  reading:   { icon: '📖', color: 'text-sky-400',    bg: 'bg-sky-500/15',     border: 'border-sky-500/30'    },
  writing:   { icon: '✍️', color: 'text-emerald-400', bg: 'bg-emerald-500/15', border: 'border-emerald-500/30'},
};
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-1.5 mb-1">
            ...
            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full border ${meta.bg} ${meta.border} ${meta.color}`}>
              {meta.icon} {t(`skills.${s.skill}.label`)}
            </span>
            {/* CEFR badge */}
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
            ...
            <span>📝 {t('placement.wordCount', { count: s.wordCount })}</span>
            <span>{formatRelative(s.timestamp, t)}</span>
          </div>
          ...
          {s.corrections && s.corrections.length > 0 && (
            ...
            >
              {expanded ? t('history.hideCorrections') : t('history.showCorrections', { count: s.corrections.length })}
            </button>
          )}
...
const FILTER_ICON: Record<Skill | 'all', string> = {
  all:       '',
  speaking:  '🎙️',
  listening: '🎧',
  reading:   '📖',
    ...
    <div className="fade-in">
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {ALL_SKILLS.map(sk => (
          ...
          >
            {sk === 'all' ? t('common.all') : `${FILTER_ICON[sk]} ${t(`skills.${sk}.label`)}`}
          </button>
        ))}
      </div>
      ...
      {filtered.length > 5 && (
        ...
        >
          {showAll ? t('history.showFewer') : t('history.showMore', { count: filtered.length - 5 })}
        </button>
      )}
